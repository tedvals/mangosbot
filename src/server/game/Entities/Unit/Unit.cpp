...
void Unit::ExecuteDelayedSwingHit(bool extra)
{
	...
	//ProcSkillsAndAuras(_damageInfo.target, _damageInfo.procAttacker, _damageInfo.procVictim, PROC_SPELL_TYPE_NONE, PROC_SPELL_PHASE_NONE, _dmgInfo.GetHitMask(), nullptr, &_dmgInfo, nullptr);
}
//NpcBot mod
    ...
    if (typeMask & MELEE_BASED_TRIGGER_MASK && procTarget)
    {
        ...
        if (hitMask & (PROC_HIT_CRITICAL | PROC_HIT_PARRY | PROC_HIT_DODGE | PROC_HIT_BLOCK))
        {
            ...
            else // For attacker
            {
                ...
                if (((hitMask & PROC_HIT_DODGE) || (hitMask & PROC_HIT_PARRY)) && GetTypeId() == TYPEID_UNIT && ToCreature()->GetBotAI() && ToCreature()->GetBotClass() == CLASS_WARRIOR)
                {
                    StartReactiveTimer(REACTIVE_OVERPOWER);
                }
                //end npcbot
