...
void RandomPlayerbotMgr::UpdateAIInternal(uint32 elapsed)
{
    ...
    sLog->outMessage("playerbot", LOG_LEVEL_INFO, "%d bots processed. %d alliance and %d horde bots added. %d bots online of %d on queue. Next check in %d seconds",
            botProcessed, allianceNewBots, hordeNewBots, playerBots.size(), bots.size(),sPlayerbotAIConfig.randomBotUpdateInterval);
